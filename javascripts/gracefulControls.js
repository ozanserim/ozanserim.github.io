...
function update(dt) {
    ...
    var newOrientation = Quat.multiply(Camera.orientation, Quat.fromVec3Degrees( { x: pitchMove, y: yawMove, z: 0 } ));
    var newOrientation = Quat.fromVec3Degrees( { x: pitch, y: yaw, z: 0 } );
    Camera.orientation = newOrientation;
    //yawFromMouse -= yawMove;

    //Vvar newPitch = MyAvatar.headPitch + pitchMove;
    ...
    var v = Vec3.multiplyQbyV(newOrientation, velocity);
    //Vec3.multiplyQbyV(newOrientation, Vec3.multiply(velocity, dt))
    ...
    Camera.position = Vec3.sum(Camera.position, Vec3.multiply(v, dt));
...
function resetCursorPosition() {
    ...
    lastX = Reticle.getPosition().x;
    lastY = Reticle.getPosition().y;
}
