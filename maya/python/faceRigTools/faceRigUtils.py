...
def transferControlToPositionGroup():
    """select all locators and then zero out control and transfer position to position groups"""


    locCtrls = cmds.ls(sl=1)
    locs = []
    for locCtrl in locCtrls:

        reposition_grp = locCtrl + "_reposition_grp"
        if not cmds.objExists(reposition_grp):
            cmds.warning("can't find " + reposition_grp)
            continue
        ...
        cmds.delete(cmds.pointConstraint(loc, reposition_grp))
        cmds.delete(cmds.orientConstraint(loc, reposition_grp))


    #now delete the locators and force the evaluation
    cmds.delete(locs)
    cmds.select(locCtrls)
